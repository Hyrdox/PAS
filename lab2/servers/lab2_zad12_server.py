    ...
    try:
        ...
        while True:
            try:
                ...
                print("[%s] Client %s sent '%s' " % (strftime("%Y-%m-%d %H:%M:%S", gmtime()), client_address, data.decode()))

                if data:
                    print("[%s] Sending back to client %s ... " % (strftime("%Y-%m-%d %H:%M:%S", gmtime()), client_address))
